...
use SurveyJsPhpSdk\Parser\Element\MatrixElementParser;
    ...
    public static function create(\stdClass $element, ?ElementConfigurationInterface $configuration): ElementInterface {
        switch ($element->type) {
            ...
            case self::MATRIX_TYPE:
                $parser = new MatrixElementParser();
                return $parser->parse($element);
            case self::MATRIX_DROPDOWN_TYPE:
                $parser = new MatrixElementParser();
                return $parser->parse($element);
            default:
                dd($element->name, $element->type);
